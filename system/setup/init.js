...
head.js(
	...
	"system/vendor/angular-translate/angular-translate.min.js",
	"system/vendor/angular-translate/angular-translate-interpolation.min.js",
	"system/vendor/angular-translate/angular-translate-storage-cookie.min.js",
	"system/vendor/angular-translate/angular-translate-storage-local.min.js",
	"system/vendor/leaflet/leaflet.js",
	"system/vendor/facebook-connect/faceGap-1.0.min.js",
	...
	"system/scripts/services/plus-geofence.js",		
	...
	//"system/scripts/services/facebook-connect-service.js",
...
head.ready(function(){
	settings = {
		app : {
			...
			data_sync : false,
			geo_watch_timeout: 30000, // 30 seconds
			geo_position_cache_limit: 180000, // 3 minutes
	...
	head.ready(function(){
		settings.languages = {};
		_.each(settings.app.languages, function(val, key){
			$.getJSON('app/includes/languages/' + val + '.lang.json', function(data){
				settings.languages[val] = data;
			})
			.fail(function(jqxhr) {
				switch(jqxhr.status){
					case 404:
						console.log('error: no lang file found for ' + val);
						break;
					case 200:
						try{
							tryJSON = JSON.parse(JSON.minify(jqxhr.responseText));
							settings.languages[val] = tryJSON;
							console.log('Remove all comments from your ' + val + ' language file for better performance');
						}catch(err){
							console.log(sprintf('Lang file found for ' + val + ', but it is not in the expected JSON format. \nError Message: %s', err.message));
						}
						break;
				}
			});
...
function loadError(jqxhr, val, type) {
	switch(jqxhr.status){
		case 404:
			if(settings.app.environment == 'development') alert(sprintf('error: no configuration file found for %s %s', type, val));
			break;
		case 200:
			try{
				tryJSON = JSON.parse(JSON.minify(jqxhr.responseText));
				...
				if(settings.app.environment == 'development') alert(sprintf('Configuration file found for %s %s, but it is not in the expected JSON format. \nError Message: %s', type, val, err.message));
			}
			break;
	}
}